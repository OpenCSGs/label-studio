...
export const FilterLine = observer(({ filter, availableFilters, index, view, sidebar, dropdownClassName, t }) => {
  const _t = t ?? ((k) => k);
          ...
          <FilterDropdown
            ...
            optionRender={({ item: { original: filter } }) => (
              <div className={cn("filterLine").elem("selector")}>
                {getColumnTitle(filter.field, filter.field.title, _t)}
                {filter.field.parent && (
                  <Tag size="small" className="filters-data-tag" color="#1d91e4" style={{ marginLeft: 7 }}>
                    {getColumnTitle(filter.field.parent, filter.field.parent.title, _t)}
                  </Tag>
                )}
              </div>
            )}
        ...
        {childFilter && (
          <>
            ...
            <div className={cn("filterLine").elem("column").mix("field child-field")}>
              <FilterDropdown
                placeholder={getColumnTitle(childFilter.field, childFilter.field.title, _t)}
                value={getColumnTitle(childFilter.field, childFilter.field.title, _t)}
                items={[
                  {
                    value: childFilter.field.title,
                    label: getColumnTitle(childFilter.field, childFilter.field.title, _t),
                  },
                ]}
                disabled={true}
