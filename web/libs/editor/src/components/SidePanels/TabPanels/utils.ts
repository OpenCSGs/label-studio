...
export const TAB_TITLE_KEYS: Record<string, string> = {
  regions: "annotation.regions",
  history: "annotation.history",
  relations: "annotation.relationsTitle",
...
const panelViews = [
  ...
  {
    name: "relations",
    title: "annotation.relationsTitle",
...
export const restoreComponentsToState = (panelData: Record<string, PanelBBox>) => {
  ...
  Object.keys(updatedPanels).forEach((panelName) => {
    ...
    panel.panelViews.forEach((view: { name: string; title?: string; component: FC<PanelProps> }) => {
      ...
      if (TAB_TITLE_KEYS[view.name]) {
        view.title = TAB_TITLE_KEYS[view.name];
      }
    });
  });
