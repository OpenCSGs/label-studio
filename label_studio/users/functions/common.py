...
def save_user_db(user_form):
    """CSGHub 二开：保存用户到 DB 并创建专属组织（模拟登录用）。user_form 为 dict，需含 'email'。"""
    from users.models import User

    password = user_form.get('password') or user_form.get('email', '')
    user = User.objects.create_user(email=user_form['email'], password=password, allow_newsletters=False)
    user.username = user.email.split('@')[0]
    user.save()
